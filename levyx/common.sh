...
generate_bdutil_config() {
  ...
  then
    PD_COUNT=1
    PD_DISK_OPTIONS=" -d"
    PD_DISK_OPTIONS+=" --master_attached_pd_type $ATTACHED_PD_TYPE --master_attached_pd_size_gb $PDSIZE --master_attached_pds_count $PD_COUNT"
    PD_DISK_OPTIONS+=" --worker_attached_pds_type $ATTACHED_PD_TYPE --worker_attached_pds_size_gb $PDSIZE --worker_attached_pds_count $PD_COUNT"
  fi
